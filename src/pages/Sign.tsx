...
import { SignerFormView } from "@/components/forms/SignerFormView";
...
export default function Sign() {
  ...
  const handleFormSubmit = async (responses: Record<string, any>, name: string) => {
    setIsSubmitting(true);
    try {
      const { data, error } = await supabase.functions.invoke('verify-signing-token', {
        body: {
      ...
      if (error || data?.error) {
        setErrorMessage(data?.error || error?.message || 'Failed to submit form');
        setPageState('error');
        return;
      }
      ...
      <div className="min-h-screen bg-gradient-to-b from-background to-muted/30 flex items-center justify-center p-4">
        <Card className="w-full max-w-md border-primary/20">
          <CardContent className="pt-8 pb-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-foreground mb-3">Submission Complete!</h2>
  ...
  // If there's a form template, show the fillable form view
  if (signingData?.formTemplate) {
    return (
      <SignerFormView
        fields={signingData.formTemplate.fields}
        pdfUrl={signingData.formTemplate.pdfUrl}
        pdfName={signingData.formTemplate.pdfName}
        requirementTitle={signingData.requirementTitle}
        organizationName={signingData.organizationName}
        organizationLogo={signingData.organizationLogo}
  ...
  // Default: simple signature acknowledgment
  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/30 flex flex-col">
      <header className="sticky top-0 z-10 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 px-4 py-4 safe-area-inset-top">
        <div className="max-w-lg mx-auto flex items-center justify-between">
          ...
          {signingData?.organizationLogo && (
            <img src={signingData.organizationLogo} alt={signingData.organizationName} className="h-10 w-10 rounded-lg object-contain" />
          )}
        </div>
      </header>
          ...
          <Card className="overflow-hidden">
            <CardHeader className="pb-4 bg-muted/30">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <CardTitle className="text-lg leading-tight mb-1">{signingData?.requirementTitle}</CardTitle>
                  {signingData?.requirementDescription && (
                    <CardDescription className="line-clamp-3">{signingData.requirementDescription}</CardDescription>
                  )}
                </div>
              </div>
            </CardHeader>
            {signingData?.attachmentUrl && (
              <CardContent className="pt-4 pb-4">
                <a href={signingData.attachmentUrl} target="_blank" rel="noopener noreferrer"
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-foreground truncate">{signingData.attachmentName || 'View Attachment'}</p>
            ...
            <CardContent className="pt-6 pb-6 space-y-6">
              <button type="button" onClick={() => setAcknowledged(!acknowledged)}
                className={`w-full flex items-start gap-4 p-4 rounded-xl border-2 transition-all touch-manipulation ${
                  acknowledged ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/30 hover:bg-muted/50'
                }`}>
                <div className={`h-6 w-6 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                  acknowledged ? 'bg-primary border-primary' : 'border-muted-foreground/30'
                }`}>
                  {acknowledged && <CheckCircle className="h-4 w-4 text-primary-foreground" />}
                </div>
              ...
              <div className="space-y-3">
                <label htmlFor="signedName" className="text-sm font-medium text-foreground block">Type your full name to sign</label>
                <Input id="signedName" placeholder="Your full legal name" value={signedName}
                  onChange={(e) => setSignedName(e.target.value)}
                  className="h-14 text-lg px-4 touch-manipulation" autoComplete="name" autoCapitalize="words" enterKeyHint="done" />
                {signedName && (
                  <div className="p-4 rounded-lg bg-muted/50 border border-border text-center">
                    <p className="text-xs text-muted-foreground mb-2">Signature preview</p>
                    <p className="text-2xl text-foreground" style={{ fontFamily: "'Brush Script MT', 'Segoe Script', 'Bradley Hand', cursive" }}>
                      {signedName}
                    </p>
                  </div>
                )}
              ...
              <Button onClick={handleSubmit} disabled={!isFormValid || isSubmitting}
                className="w-full h-14 text-base font-semibold touch-manipulation" size="lg">
                {isSubmitting ? (<><Loader2 className="h-5 w-5 mr-2 animate-spin" />Submitting...</>) : 'Sign & Acknowledge'}
              </Button>
      ...
      <footer className="border-t bg-background px-4 py-4 mt-auto safe-area-inset-bottom">
        <div className="max-w-lg mx-auto text-center">
          <p className="text-xs text-muted-foreground">Secured by <span className="font-semibold">LedgerStash</span></p>
        </div>
      </footer>
    </div>
  );
