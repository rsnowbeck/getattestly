...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const safeInviteUrl = escapeAttr(inviteUrl);
          ...
          <tr>
            <td style="padding: 0 32px 24px 32px;">
              <p style="margin: 0 0 16px 0; font-size: 16px; line-height: 24px; color: #3f3f46;">
                ${safeInviterName ? `<strong>${safeInviterName}</strong> has invited you` : "You've been invited"} to join <strong>${safeOrgName}</strong> on LedgerStash as a <strong>${roleLabel}</strong>.
              </p>
              ...
              <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                <tr>
                  <td align="center">
                    <a href="${safeInviteUrl}" target="_blank" style="display: inline-block; padding: 14px 32px; background-color: #18181b; color: #ffffff; font-size: 16px; font-weight: 600; text-decoration: none; border-radius: 8px;">
                      Accept Invitation
                    </a>
                  </td>
                </tr>
